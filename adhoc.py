...
cwd = os.getcwd()
cwd = str(cwd)
pre_asp_learner = cwd + '/Adhoc_Architecture/ASP/learner_pre1.sp'
asp_learner =  cwd + '/Adhoc_Architecture/ASP/learner1.sp'
...
class adhoc_Agent:
    ...
    # calculates terms that will be used to query an ASP learner to make decisions.
    ...
    # Look ahead using the models
    # calculates terms to predict future actions.
    ...
    def run_ASP_learner(self, input, input1, pre_asp_split, display_marker_index):
        ...
        f1.write(asp)
        f1.close()
        answer = subprocess.check_output('java -jar '+ cwd + '/Adhoc_Architecture/ASP/sparc.jar ' +asp_learner+' -A',shell=True)
    ...
    # Prepares terms needed for ASP logic by extracting information from the current state.
    ...
    # Prepares terms for predicting future actions.
    def get_future_terms(self, step, input):
        print(f"inside get_future {self.uniform_numbers}")
    ...
    # Uses pre-trained machine learning models to predict actions for other agents.
    def predict_class(self, state):

        predict_actions = inference.get_predictions(state)
        
        return predict_actions
    

    # these two are used to return a list of teammate and opponent objects from a given list of objects.
